...
func TestShadowPrimaryResultUnaffectedByShadowError(t *testing.T) {
	...
	s := NewShadowAuthorizer(primary, shadow, ShadowOptions{
		SampleRate:    1,
		OnShadowError: func(string) { shadowErrs++ },
	})
	...
	waitFor(t, func() bool { return shadowErrs == 1 }, "shadow error observation")
}
...
func TestShadowMismatchAllowDeny(t *testing.T) {
	...
	s := NewShadowAuthorizer(primary, shadow, ShadowOptions{
		...
		OnMismatch: func(tenantID, category string) {
			...
			mismatches = append(mismatches, category)
		},
	})
	...
	waitFor(t, func() bool { return len(mismatches) == 1 }, "allow/deny mismatch")
	if mismatches[0] != ShadowMismatchAllowDeny {
		t.Fatalf("category = %q, want allow_deny_mismatch", mismatches[0])
	}
}
...
func TestShadowMismatchAgreementIsNotMismatch(t *testing.T) {
	...
	s := NewShadowAuthorizer(primary, shadow, ShadowOptions{
		SampleRate: 1,
		OnMismatch: func(string, string) { mismatches++ },
	})
	...
	if mismatches != 0 {
		t.Fatalf("agreement must not be a mismatch, got %d", mismatches)
	}
}
...
func TestShadowMismatchErrorCategory(t *testing.T) {
	...
	waitFor(t, func() bool { return len(mismatches) == 1 }, "error mismatch")
	if mismatches[0] != ShadowMismatchError {
		t.Fatalf("category = %q, want error_mismatch", mismatches[0])
	}
}
...
func TestShadowFallbackAlsoReportsErrorMismatch(t *testing.T) {
	...
	if len(mismatches) != 1 || mismatches[0] != ShadowMismatchError {
		t.Fatalf("primary error vs shadow success must be an error mismatch, got %v", mismatches)
	}
}
...
func TestShadowBothErroredIsNotMismatch(t *testing.T) {
	...
	if mismatches != 0 {
		t.Fatalf("both errored is consistent, not a mismatch, got %d", mismatches)
	}
}
